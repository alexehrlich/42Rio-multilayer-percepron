...
class Model:
    def __init__(self, model_type: str):
        ...
        self.train_acc_values: List[float] = []
        self.val_acc_values: List[float] = []
...
class Network:
    def __init__(self, loss_function: Callable[[np.ndarray, np.ndarray], float]):
        ...
        self.loss_function_derivative: Callable[[np.ndarray, np.ndarray], np.ndarray] = loss_deriv[loss_function]
    ...
    def backpropagation(self, target: np.ndarray) -> None:
        """
			Pass the delta backwards through the net. The layer fills its nabla 
        ...
        if self.loss_function == categorical_cross_entropy_loss:
            delta = self.layers[-1].activations - one_hot(target, self.layers[-1].nodes)
        else:
    ...
    def learn_parameter(self, eta: float, batch_size: int) -> None:
        """
    ...
    def learn_mini_batch(self, 
		mini_batch: List[Tuple[np.ndarray, int]], 
		eta: float, 
		batch_size: int
		) -> Tuple[float, int]:
        """
...
class Layer:
    def __init__(self, 
		...
		weight_initialization: Callable[[Any, int], None]
		):
        """
        ...
        self.weight_initialization: Callable[[Any, int], None] = weight_initialization
        self.activation: Callable[[np.ndarray], np.ndarray] = activation
        self.derivative_activation: Callable[[np.ndarray], np.ndarray] = func_deriv[activation]
        self.weights: Optional[np.ndarray] = None
        self.nabla_w: Optional[np.ndarray] = None
        self.biases: np.ndarray = np.zeros((nodes, 1))
        self.nabla_b: np.ndarray = np.zeros((nodes, 1))
        self.z: np.ndarray = np.zeros((nodes, 1))
        self.activations: np.ndarray = np.zeros((nodes, 1))
