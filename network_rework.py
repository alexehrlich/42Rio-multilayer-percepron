...
class Network:
	...
	def learn_parameter(self, eta):
		for layer in self.layers:
			self.biases -= eta * layer.nabla_b
			self.weights -= eta * layer.nabla_w
	...
	def fit(self, training_data, epochs, eta, validation_data = None):
		...
		for epoch in np.arange(0, epochs):
			loss = 0
			#TODO: shuffle the training data, SEED
			for featrues, target in training_data:
				loss += self.backpropagation(featrues, target)
